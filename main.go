...
func (e *generator) processMessage(
	...
	for i, field := range iter(msg.Fields()) {
		...
		arbitraryType := field.Kind() == protoreflect.MessageKind && !field.IsMap() &&
			field.Message() != nil && string(field.Message().ParentFile().Package()) != string(field.ParentFile().Package())
		f := Field{
			Name:          name,
			Alias:         alias,
			Type:          typ,

...
type Field struct {
	...
	Type             string
	Optional         bool
	ArbitraryType    bool // true when the type is not natively supported by Pydantic (e.g. protobuf messages)
	OneOf            *OneOf
	LeadingComments  []string
	TrailingComments []string
}
...
func (m Message) HasModelConfig() bool {
	return m.HasAlias() || m.HasArbitraryTypes()
}

func (m Message) HasArbitraryTypes() bool {
	for _, f := range m.Fields {
		if f.ArbitraryType {
			return true
		}
...
func (e *generator) processMessage(
	...
	for i, field := range iter(msg.Fields()) {
		...
		arbitraryType := false
		if field.Kind() == protoreflect.MessageKind && !field.IsMap() && field.Message() != nil {
			isExternal := string(field.Message().ParentFile().Package()) != string(field.ParentFile().Package())
			_, isWKT := wellKnownTypes[string(field.Message().FullName())]
			arbitraryType = isExternal && !isWKT
		}
		f := Field{
			Name:          name,
			Alias:         alias,

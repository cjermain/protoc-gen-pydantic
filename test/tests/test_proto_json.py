...
def test_nested_model_omits_defaults():
    """ProtoJSON defaults apply to nested models: zero-value fields are omitted
    and field aliases are used when serializing via model_dump and model_dump_json."""
    ...
    result2_dict = s2.model_dump()
    assert "bool" in result2_dict
    assert "bool_" not in result2_dict

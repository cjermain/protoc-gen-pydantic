...
{{template "renderEnum" (dict "Enum" . "Indent" "" "Config" $config)}}{{- end }}{{- range .Messages }}


{{template "renderMessage" (dict "Message" . "Indent" "" "Config" $config)}}{{- end -}}
...
{{ end }}
{{template "renderEnum" (dict "Enum" $e "Indent" $bi "Config" $config)}}{{- end }}{{- if and (or $m.LeadingComments $m.HasAlias $m.NestedEnums) $m.NestedMessages }}
{{ end }}{{- range $i, $nm := $m.NestedMessages }}{{ if $i }}
{{ end }}
{{template "renderMessage" (dict "Message" $nm "Indent" $bi "Config" $config)}}{{- end }}
...
func buildProtoTypesContent(needed map[string]bool, config GeneratorConfig, dirCustomOptionFields []CustomOptionField) string {
	...
	if needed["_ProtoEnum"] && !config.UseNoneUnionSyntaxInsteadOfOptional {
		b.WriteString("from typing import Optional as _Optional\n")
	}
	b.WriteString("\n")
	...
	// Enum infrastructure: _EnumValueOptions + _ProtoEnum.
	if needed["_ProtoEnum"] {
		b.WriteString(buildProtoEnumContent(config, dirCustomOptionFields))
	}
...
func buildProtoEnumContent(config GeneratorConfig, dirCustomOptionFields []CustomOptionField) string {
	optType := "_EnumValueOptions | None"
	if !config.UseNoneUnionSyntaxInsteadOfOptional {
		optType = "_Optional[_EnumValueOptions]"
	}

	var b strings.Builder

	b.WriteString("\n\n@_dataclass(frozen=True)\nclass _EnumValueOptions:\n")
	b.WriteString("    deprecated: bool = False\n")
	b.WriteString("    debug_redact: bool = False\n")
	for _, f := range dirCustomOptionFields {
		if config.UseNoneUnionSyntaxInsteadOfOptional {
			b.WriteString("    " + f.Name + ": " + f.PythonType + " | None = None\n")
		} else {
			b.WriteString("    " + f.Name + ": _Optional[" + f.PythonType + "] = None\n")
		}
	}

	if config.UseIntegersForEnums {
		b.WriteString("\n\nclass _ProtoEnum(int, _Enum):\n")
		b.WriteString("    number: int\n")
		b.WriteString("    _options_: _EnumValueOptions\n")
		b.WriteString("\n    def __new__(cls, value: int, options: " + optType + " = None):\n")
		b.WriteString("        obj = int.__new__(cls, value)\n")
		b.WriteString("        obj._value_ = value\n")
		b.WriteString("        obj.number = int(value)\n")
		b.WriteString("        obj._options_ = options if options is not None else _EnumValueOptions()\n")
		...
		b.WriteString("        number: int = 0,\n")
		b.WriteString("        options: " + optType + " = None,\n")
		b.WriteString("    ):\n")
		b.WriteString("        obj = str.__new__(cls, value)\n")
		b.WriteString("        obj._value_ = value\n")
		b.WriteString("        obj.number = number\n")
		b.WriteString("        obj._options_ = options if options is not None else _EnumValueOptions()\n")
		b.WriteString("        return obj\n")
	}

	b.WriteString("\n    @property\n")
	b.WriteString("    def options(self) -> _EnumValueOptions:\n")
	b.WriteString("        return self._options_\n")
